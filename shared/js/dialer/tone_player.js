...
var TonePlayer = {
  ...
  init: function tp_init(channel) {
    ...
    this._maybeTrashAudio = (function tp_maybeTrashAudio() {
       ...
       * or current chanel is 'telephony' but no active call */
      if ((document.hidden &&
    ...
    this._releaseAudioForCall = (function tp_releaseAudioForCall() {
      var callIsActive = telephony && (telephony.calls.length ||
          telephony.conferenceGroup.calls.length);
      if (!callIsActive) {
        setTimeout(() => {
          this._trashAudio();
        }, 100);//To make sure callscreen has finished to play tone
      }
    }).bind(this);

    window.addEventListener('visibilitychange', this._maybeTrashAudio);
    telephony && telephony.addEventListener('callschanged',
                                            this._releaseAudioForCall);
    telephony && telephony.conferenceGroup.addEventListener('callschanged',
        this._releaseAudioForCall);
  ...
  _trashAudio: function tp_trashAudio() {
    this.stop();
    if (this._audioContext && this._audioContext.requestCleanAfterRelease) {
      this._audioContext.requestCleanAfterRelease();
    }
    this._audioContext = null;
  },
