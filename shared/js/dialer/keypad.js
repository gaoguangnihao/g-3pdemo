...
var KeypadManager = {
  ...
  get keyboardContainer() {
    delete this.keyboardContainer;
  ...
  _touchStart: function kh_touchStart(key) {
    ...
    // Manage long press
    var isSpeedDial = this._isSpeedDialKey(key);
    if (((key == '0' || key == '*' || isSpeedDial) && !this._onCall) || key == 'delete') {
  ...
  enableKeyPad: function kh_enableKeyPad() {
    this.enableKeypad = true;
  },

  disableKeyPad: function kh_disableKeyPad() {
  ...
  keyHandler: function kh_keyHandler(event) {
    ...
    // Event.preventDefault cancells processing of Volume Up/Volume Down buttons
    // in HardwareButtons.prototype.handleEvent in app/system/js/hardware_buttons.js
    // so Volume pop-up is not appeared.
    ...
    if (event.type == 'keydown' || event.type == 'keyup') {
        key = event.key;
        if (key !== 'Backspace' && key !== 'Clear' && !this._isCorrectKeyValue(key)) {
          console.log('incorrect key was found ' + key);
          return;
        }
    } else {
    ...
    (key === 'Backspace' || key === 'Clear') && (key = 'delete');
    ...
    if (key === '#' && !this._onCall) {
      if (this._phoneNumber === '*#06#' || this._phoneNumber === '*#3228#' || this._phoneNumber === '*#0606#') {
  ...
  /**
   * Indicates whether the given key is one of speed dial keys
   * that can be assigned to some phone (and contact).
   */
  _isSpeedDialKey: function(key) {
    return (['2', '3', '4', '5', '6', '7', '8', '9'].indexOf(key) != -1);
  },

  _runSpeedDialHandler: function(speedDial) {
    window.dispatchEvent(new CustomEvent('speedDialKeyPressed', {
      detail: {
        speedDial: speedDial
      }
    }));
  ...
  _isCorrectKeyValue: function kh_isCorrectKeyValue(n) {
      return (!isNaN(n) || n == '*' || n == '#' || n == ',' || n == '+');
  },
  ...
  insertChar: function kh_insertChar(src, pos, word) {
    return src.substring(0, pos) + word + src.substring(pos, src.length);
  },

  separatePhoneNumber: function kh_separatePhoneNumber(number) {
    var postion = null;
    if (number[0] == '1') {
      postion = new Array(1,5,9);
    } else if (number[0] == '+') {
      this.phoneNumberView.value = number;
      return;
    } else {
      postion = new Array(3, 7);
    }
    var separeteNumber = number;
    for (var i = 0; i < postion.length; i++) {
      if (separeteNumber.length > postion[i])
        separeteNumber =
            this.insertChar(separeteNumber, postion[i], '-');
    }
    this.phoneNumberView.value = separeteNumber;
  },
  ...
  _updatePhoneNumberView: function kh_updatePhoneNumberview(ellipsisSide,
    ...
    if (this._onCall) {
      this.replacePhoneNumber(phoneNumber);
    } else {
      // var visibility;
      ...
      this.separatePhoneNumber(phoneNumber);

      FontSizeManager.adaptToSpace(
        FontSizeManager.DIAL_PAD, this.phoneNumberView, forceMaxFontSize,
        ellipsisSide);

      document.dispatchEvent(new CustomEvent('phoneNumberViewChanged', {
        detail: {
          value: this.phoneNumberView.value
        }
      }));
    }
  ...
  replacePhoneNumber: function kh_replacePhoneNumber(phoneNumber) {
    if (this._onCall && !CallsHandler.isVideoCall()) {
      CallScreen.showDtmfNumber(phoneNumber);
    }
  },
