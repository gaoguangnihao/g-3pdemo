...
var MediaDB = (function() {
  ...
  MediaDB.prototype = {
    ...
    deleteFile: function deleteFile(filename, callback) {
      if (this.state !== MediaDB.READY) {
        throw Error('MediaDB is not ready. State: ' + this.state);
      }

      var storage = navigator.getDeviceStorage(this.mediaType);
      var req = storage.delete(filename);
      req.onerror = function(e) {
    ...
    //
    addFile: function addFile(filename, file, callback, errcallback) {
      if (this.state !== MediaDB.READY) {
        throw Error('MediaDB is not ready. State: ' + this.state);
      }
      ...
      function save() {
        ...
        request.onsuccess = function() {
          console.log('MediaDB: operation successfully');
          if (callback) {
            callback();
          }
        };
    ...
    function parseMetadata(file, filename, oldMetadata) {
      ...
      var fileinfo = {
        ...
        date: file.lastModifiedDate ?
          file.lastModifiedDate.getTime() : Date.now(),
        firstParseTime: file.firstParseTime ?
          file.firstParseTime : Date.now()
      };
