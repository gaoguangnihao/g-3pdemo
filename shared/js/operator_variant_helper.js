...
OperatorVariantHelper.prototype = {
  ...
  // The mozSettings key for the saved mvnoInfo.
  get MVNO_SETTINGS_KEY() {
    return 'operatorvariant.mvno';
  },
  ...
  checkMvnoChange: function() {
    var iccCard = this._iccCard;
    return new Promise((resolve, reject) => {
      var mvnoChange = false;
      var transaction = this.settings.createLock();

      var mvnoInfoRequest = transaction.get(this.MVNO_SETTINGS_KEY);
      mvnoInfoRequest.onsuccess = () => {
        var mvnoInfos = mvnoInfoRequest.result[this.MVNO_SETTINGS_KEY];
        var allMatchPromise = [];
        var mvnoInfo = (mvnoInfos && mvnoInfos[this._iccCardIndex]) || [];
        for (var i = 0; i < mvnoInfo.length; i++) {
          var matchPromise = new Promise(function(_resolve, _reject) {
            var matchInfo = mvnoInfo[i];
            var request = iccCard.matchMvno(matchInfo['mvnoType'],
                                            matchInfo['mvnoMatchData']);
            request.onsuccess = () => {
              var match = request.result;
              if (match !== matchInfo['match']) {
                mvnoChange = true;
              }
              _resolve();
            };
          ...
          allMatchPromise.push(matchPromise);
        }
        Promise.all(allMatchPromise).then(() => {
          resolve(mvnoChange);
        });
      };
      mvnoInfoRequest.onerror = () => {
        resolve(true);
      };
    });
  },
  ...
  checkICCInfo: function() {
    ...
    var plmnChanged = (mcc !== this._iccSettings.mcc ||
      mnc !== this._iccSettings.mnc);
    ...
    if (plmnChanged) {
      ...
      this.checkMvnoChange().then((changed) => {
        if (changed) {
          _applyNewApn();
        } else {
