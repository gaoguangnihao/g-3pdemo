...
(function (exports) {
  ...
  var backCallBack = [];
  var softkey_enabled = false;
  var HelperEvent = function (e) {
    switch (e.type) {
    case 'screenchange':
  ...
  var SoftkeyHelper = {
    ...
    init: function (params, callback) {
      ...
      softkey.show();
      backCallBack = callback || function () {};
      softkey_enabled = true;
    },
    updateSoftkey: function (params, keyStyle, name) {
      switch (keyStyle) {
      case '1':
    ...
    hide: function (name) {
      ...
      for (var i = 0; i < this.applist.length; i++) {
        if (this.applist[i].name == name) {
          ...
          if (i == 0 && this.applist.length > 0 && this.applist[0].appliacation.front != undefined) {
    ...
    register: function (app, name) {
      ...
      if (this.applist.length > 0 && this.applist[0].appliacation.back != undefined) {
